...
const PatrimoniosContext = createContext<PatrimoniosContextData | undefined>(undefined);
...
export const PatrimoniosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createPatrimonio = useCallback(async (data: PatrimonioCreate): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const updatePatrimonio = useCallback(async (id: number, data: PatrimonioUpdate): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
    ...
    filtrados.sort((a, b) => {
      ...
      switch (ordenacao.campo) {
        ...
        case 'responsavel_nome':
          aVal = usuarios.find(u => u.id === a.responsavel_id)?.username || '';
          bVal = usuarios.find(u => u.id === b.responsavel_id)?.username || '';
          break;
        default:
