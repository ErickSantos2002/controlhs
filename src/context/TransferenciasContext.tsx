...
const TransferenciasContext = createContext<
  TransferenciasContextData | undefined
>(undefined);
...
export const TransferenciasProvider: React.FC<{
  ...
  const podeAprovar = useCallback(
    (transferencia: Transferencia): boolean => {
      ...
      if (role === 'gestor') {
        ...
        return (
          setor_id === transferencia.setor_origem_id ||
  ...
  const podeEfetivar = useCallback(
    (transferencia: Transferencia): boolean => {
  ...
  const verificarTransferenciaPendente = useCallback(
    (patrimonio_id: number): boolean => {
  ...
  const createTransferencia = useCallback(
    async (data: TransferenciaCreate): Promise<void> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updateTransferencia = useCallback(
    async (id: number, data: TransferenciaUpdate): Promise<void> => {
      try {
        setLoading(true);
        setError(null);
        ...
        setError(
          err.response?.data?.detail || 'Erro ao atualizar transferência',
        );
        throw err;
      } finally {
        setLoading(false);
  ...
  const kpis = useMemo((): TransferenciasKPIs => {
    ...
    const pendentes = transferenciasComStatus.filter(
      (t) => t.status === 'pendente',
    ).length;
    ...
    filtradas.sort((a, b) => {
      ...
      switch (ordenacao.campo) {
        ...
        case 'responsavel_origem_nome':
          aVal =
            usuarios.find((u) => u.id === a.responsavel_origem_id)?.username ||
            '';
          bVal =
            usuarios.find((u) => u.id === b.responsavel_origem_id)?.username ||
            '';
          break;
        case 'responsavel_destino_nome':
          aVal =
            usuarios.find((u) => u.id === a.responsavel_destino_id)?.username ||
            '';
          bVal =
...
export const useTransferencias = () => {
  ...
  if (!context) {
    throw new Error(
      'useTransferencias deve ser usado dentro de TransferenciasProvider',
    );
  }
  return context;
};